...
if( $in{'delete'} ) {
	...
	if( $in{'d'} ) {
		...
		foreach $d (sort { $b <=> $a } @d) {
			...
			$fw->DeleteConntrackPreroute($idx);
		}
	} elsif( $idx ne '' ) {
		$whatfailed = $text{save_conntrack_error_title1};
		$fw->DeleteConntrackPreroute($idx);
...
if( $idx ne $newIdx ) { $fw->MoveConntrackPreroute( $idx, $newIdx ); $idx=$newIdx; }
$fw->SaveFirewall();
&redirect( 'list_rawrules.cgi'.($in{'delete'} ? '' : "?table=conntrackpreroute&idx=$idx") );
