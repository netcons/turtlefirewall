...
if( -f "./setup/turtlefirewall" ) {
	&ui_print_header( undef, $text{'title'}, "" );
	print &ui_alert_box('This is the first execution of Turtle Firewall, you need to install/update startup scripts.', 'warn');
	print "<br>\n";
	print &ui_form_start("setup.cgi","post");
	print &ui_submit("Install Turtle Firewall Startup scripts","install");
	print &ui_form_end();
	print "<br>\n";
	print &ui_alert_box('Remember to install xt_ndpi, xt_geoip and xt_ratelimit kernel modules.', 'warn');
	print "<br>\n";
	&ui_print_footer('/',$text{'index'});
	exit;
}
...
%icons = ( 
	...
	'CLAMPMSS' => { IMAGE => '<img src=images/grey-clampmss.png hspace=4>' },
	'CLAMPMSS_A' => { IMAGE => '<img src=images/clampmss.png hspace=4>' },
